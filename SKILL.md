...
# Explicit site override (full name)
$WP --site=my-site plugin list

# Shorthand option (same as --site=)
$WP -s my-site plugin list

# Fuzzy matching (partial name, case-insensitive)
$WP -s my plugin list  # matches "my-site", "myapp", "amy-site", etc.
$WP -s form plugin list  # matches "formipay", "formula", "platform", etc.

# List all sites with running/halted status
$WP --list
```

**If auto-detection fails** (CWD is not inside any Local site) and no site option is given, the script prints available sites. Ask the user which site to target, or use `-s <name>` or `--site=<name>`.
